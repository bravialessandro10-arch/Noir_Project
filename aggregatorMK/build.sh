...
STEP_START=$(date +%s%N)
bb write_vk --verifier_target evm -b ./target/aggregatorMK.json -o ./target
STEP_END=$(date +%s%N)
...
STEP_START=$(date +%s%N)
bb write_solidity_verifier --verifier_target evm -k ./target/vk -o ./target/VerifierAggregatorMK.sol
STEP_END=$(date +%s%N)
...
STEP_START=$(date +%s%N)
cd ../hardhat_test
node scripts/genera_proofaggregataMK.js
cd ../aggregatorMK
STEP_END=$(date +%s%N)
