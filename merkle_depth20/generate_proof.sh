...
VK_START=$(date +%s%N)
bb write_vk -b ./target/merkle_depth20.json -o ./target --oracle_hash keccak
VK_END=$(date +%s%N)
...
WITNESS_SIZE=$(stat -f%z ./target/merkle_depth20.gz 2>/dev/null || stat -c%s ./target/merkle_depth20.gz)
VERIFIER_SIZE=$(stat -f%z ./target/Verifier.sol 2>/dev/null || stat -c%s ./target/VerifierMK20.sol)
