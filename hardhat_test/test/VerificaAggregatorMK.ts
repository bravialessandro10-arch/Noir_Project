...
describe("HonkVerifier On-Chain Verification", function () {
  it("Verifica proof zk on-chain", async function () {
    ...
    const TranscriptLibFactory = await ethers.getContractFactory("contracts/VerifierAggregatorMK.sol:ZKTranscriptLib");
    ...
    const VerifierFactory = await ethers.getContractFactory("contracts/VerifierAggregatorMK.sol:HonkVerifier", {
      libraries: {
    ...
    const proofPath = path.join(__dirname, "../../aggregatorMK/target/proof_hex.txt");  
    const proof = fs.readFileSync(proofPath, "utf8").trim();

    // Carica publicInputs array da file (parse JSON)
    const publicInputsPath = path.join(__dirname, "../../aggregatorMK/target/public_inputs_array.txt");
