...
echo "=============================================="
echo "Merkle Tree Configuration"
echo "=============================================="
echo "DEPTH: $DEPTH"
echo "2^DEPTH: 2^$DEPTH"
...
VK_START=$(date +%s%N)
bb write_vk -b ./target/merkle_depth16.json -o ./target --oracle_hash keccak
VK_END=$(date +%s%N)
...
bb write_solidity_verifier -k ./target/vk -o ./target/VerifierMK16.sol
...
bb prove -b ./target/merkle_depth16.json \
    -w ./target/merkle_depth16.gz \
    -o ./target \
    --oracle_hash keccak
PROOF_END=$(date +%s%N)
...
WITNESS_SIZE=$(stat -f%z ./target/merkle_depth16.gz 2>/dev/null || stat -c%s ./target/merkle_depth16.gz)
VERIFIER_SIZE=$(stat -f%z ./target/Verifier.sol 2>/dev/null || stat -c%s ./target/VerifierMK16.sol)
