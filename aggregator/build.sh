...
STEP_START=$(date +%s%N)
bb write_solidity_verifier --verifier_target evm -k ./target/vk -o ./target/VerifierAggregatorSudoku.sol
STEP_END=$(date +%s%N)
...
STEP_START=$(date +%s%N)
cd ../hardhat_test
node scripts/genera_proofaggregatafix.js
cd ../aggregator
STEP_END=$(date +%s%N)
...
STEP_START=$(date +%s%N)
bb prove --scheme ultra_honk --verifier_target evm -b ./target/aggregator.json -w ./target/aggregator.gz -o ./target
STEP_END=$(date +%s%N)
