...
STEP_START=$(date +%s%N)
bb write_vk --verifier_target evm -b ./target/aggregator10.json -o ./target
STEP_END=$(date +%s%N)
...
STEP_START=$(date +%s%N)
bb write_solidity_verifier --verifier_target evm -k ./target/vk -o ./target/Verifieraggregator10.sol
STEP_END=$(date +%s%N)
...
STEP_START=$(date +%s%N)
cd ../hardhat_test
node scripts/genera_proofaggregata10.js
cd ../aggregator10
STEP_END=$(date +%s%N)
...
STEP_START=$(date +%s%N)
bb prove --scheme ultra_honk --verifier_target evm -b ./target/aggregator10.json -w ./target/aggregator10.gz -o ./target
STEP_END=$(date +%s%N)
